...
from pycroscope.options import render_config_options_markdown
...
nitpick_ignore = [
    ...
    ("py:class", "pycroscope.input_sig.AnySig"),
    ("py:class", "pycroscope.input_sig.ActualArguments"),
    ("py:class", "pycroscope.input_sig.FullSignature"),
...
_OPTION_DOCS_PLACEHOLDER = "<!-- AUTOGENERATED_CONFIG_OPTIONS -->"
_PYCROSCOPE_PACKAGE_DIR = Path(__file__).resolve().parent.parent / "pycroscope"


def _iter_pycroscope_submodules() -> list[str]:
    def should_include(module_name: str) -> bool:
        leaf = module_name.rsplit(".", 1)[-1]
        return not (
            leaf.startswith("test") or leaf.endswith("_test") or leaf.endswith("_tests")
        )

    return sorted(
        module_info.name
        for module_info in pkgutil.walk_packages(
            [str(_PYCROSCOPE_PACKAGE_DIR)], prefix="pycroscope."
        )
        if should_include(module_info.name)
    )


def _load_option_modules() -> None:
    for module_name in _iter_pycroscope_submodules():
        import_module(module_name)


def _inject_generated_config_options(
    app: object, docname: str, source: list[str]
) -> None:
    if docname != "configuration":
        return
    if _OPTION_DOCS_PLACEHOLDER not in source[0]:
        return
    _load_option_modules()
    source[0] = source[0].replace(
        _OPTION_DOCS_PLACEHOLDER,
        render_config_options_markdown(
            excluded_options=frozenset({"paths", "import_paths"}),
...
def setup(app: object) -> None:
    app.connect("source-read", _inject_generated_config_options)
