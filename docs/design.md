...
Type compatibility checks are implemented in `pycroscope.relations` via `has_relation(...)`. The helper `Value.can_assign(...)` remains as a compatibility wrapper for external callers and answers the question: if we expect a value `X`, is it legal to pass a value `Y` instead? For example, `TypedValue(int).can_assign(KnownValue(1), ctx)` succeeds because `1` is a valid `int`, but `TypedValue(int).can_assign(KnownValue("1"), ctx)` fails because `"1"` is not. To support generic inference, successful checks return a (possibly empty) dictionary that maps type variables to inferred bounds.
...
Pycroscope can also look for unused class attributes with `--find-unused-attributes`. This reuses `ClassAttributeChecker`'s attribute-read tracking and resolves each recorded read against the receiver class and its examined subclasses. That means inherited attributes still count as used, while subclass overrides can be reported separately from the base-class member they replace. The checker also records when a concrete class is proven assignable to a protocol and treats that class like an additional subclass of the protocol for unused-member purposes, which avoids false positives for protocol-based helper objects. The check only reports attributes declared in user code, not framework-generated entries from things like `Enum` or `NamedTuple`, and it ignores `TypedDict` fields because those are normally consumed through key access rather than attribute access. If you need to suppress additional framework-specific patterns, configuration can provide predicate hooks such as ones that ignore `visit_*` methods on `ast.NodeVisitor` subclasses or helper classes that live only in test modules.

There is also an opt-in `--find-unused-call-patterns` check for functions and methods whose declared call shapes are only partially exercised by the codebase. It reuses pycroscope's successful-call binding data, so it sees the same bound arguments that type checking accepted after defaults, keyword binding, and overload selection. Today it reports a few deliberately conservative cases: optional parameters that are always omitted, boolean parameters that are only called with one literal value, and union-typed parameters whose observed calls use only a strict subset of the declared union members. Calls that flow through `*args`, `**kwargs`, `Any`, or other ambiguous patterns suppress the report for that parameter rather than guessing. This check is still experimental and has known false positives and bugs.
