...
def test_class_key_from_intersection_with_consistent_key() -> None:
    value = IntersectionValue(
        (
            TypedValue(class_owner_from_key("mod.Base")),
            SubclassValue(TypedValue(class_owner_from_key("mod.Base"))),
        )
    )
    assert _class_key_from_value(value) == class_owner_from_key("mod.Base")
...
def test_lookup_declared_symbol_with_owner_handles_synthetic_base() -> None:
    ...
    base = SyntheticClassObjectValue(
        "Base", TypedValue(class_owner_from_key("mod.Base"))
    )
    child = SyntheticClassObjectValue(
        "Child", TypedValue(class_owner_from_key("mod.Child"))
    )
    checker.register_synthetic_class(base)
    checker.register_synthetic_class(child)
    checker.make_type_object(class_owner_from_key("mod.Base")).set_declared_symbol(
        "x", ClassSymbol(annotation=TypedValue(int))
    )
    checker.make_type_object(class_owner_from_key("mod.Child")).set_direct_bases(
        (TypedValue(class_owner_from_key("mod.Base")),)
    )

    match = lookup_declared_symbol_with_owner(
        class_owner_from_key("mod.Child"), "x", checker
    )
    assert match is not None
    owner, symbol = match
    assert owner == class_owner_from_key("mod.Base")
...
def test_synthetic_mro_prefers_non_virtual_duplicate_base() -> None:
    ...
    checker = Checker()
    checker.make_type_object(class_owner_from_key("mod.A")).set_direct_bases(
        (TypedValue(list),)
    )
    checker.make_type_object(class_owner_from_key("mod.B")).set_direct_bases(
        (TypedValue(Sequence),)
    )
    checker.make_type_object(class_owner_from_key("mod.Child")).set_direct_bases(
        (
            TypedValue(class_owner_from_key("mod.A")),
            TypedValue(class_owner_from_key("mod.B")),
        )
    )

    entries = {
        entry.tobj.typ: entry
        for entry in checker.make_type_object(
            class_owner_from_key("mod.Child")
        ).get_mro()
        if entry.tobj is not None
    }

    assert entries[class_owner_from_key("mod.A")].is_virtual is False
...
def test_synthetic_type_object_tracks_dataclass_fields_without_initializers() -> None:
    ...
    checker.register_synthetic_class(base)
    checker.register_synthetic_class(child)
    checker.make_type_object(class_owner_from_key("mod.Base")).set_dataclass_info(
        dataclass_info
    )
    checker.make_type_object(class_owner_from_key("mod.Child")).set_dataclass_info(
        dataclass_info
    )
    checker.make_type_object(class_owner_from_key("mod.Child")).set_direct_bases(
        (TypedValue(class_owner_from_key("mod.Base")),)
    ...
    checker.make_type_object(class_owner_from_key("mod.Child")).set_declared_symbol(
        "b",
...
def test_type_object_exposes_synthetic_namedtuple_metadata() -> None:
    checker = Checker()
    checker.make_type_object(class_owner_from_key("mod.Base")).set_namedtuple_fields(
    ...
    type_object = checker.make_type_object(class_owner_from_key("mod.Child"))
...
def test_synthetic_declared_symbol_overrides_raw_attribute_value() -> None:
    ...
    synthetic = SyntheticClassObjectValue(
        "Impl", TypedValue(class_owner_from_key("mod.Impl"))
    )
    checker.register_synthetic_class(synthetic)
    checker.make_type_object(class_owner_from_key("mod.Impl")).set_declared_symbol(
        "attr",
        ClassSymbol(
    ...
    symbol = checker.make_type_object(
        class_owner_from_key("mod.Impl")
    ).get_declared_symbol("attr")
    assert symbol is not None
...
def test_type_object_declared_symbols_are_canonical_for_synthetic_class() -> None:
    ...
    checker.register_synthetic_class(synthetic)
    type_object = checker.make_type_object(class_owner_from_key("mod.Impl"))
    ...
    checker.register_synthetic_protocol_members(
        class_owner_from_key("mod.Impl"), {"extra"}
    )
...
def test_runtime_and_synthetic_type_objects_keep_declared_symbols_separate() -> None:
    ...
    checker = Checker()
    synthetic_key = class_owner_from_key(f"{Impl.__module__}.{Impl.__qualname__}")
...
def test_get_enum_value_type_falls_back_to_declared_value_annotation() -> None:
    ...
    checker = Checker()
    synthetic = checker.make_synthetic_class(class_owner_from_key("mod.Color"))
    type_object = checker.make_type_object(class_owner_from_key("mod.Color"))
    ...
    assert synthetic.class_type == TypedValue(class_owner_from_key("mod.Color"))
...
class TestSyntheticType(TestNameCheckVisitorBase):
    ...
    def test_protocol_inheritance(self):
        ...
        def capybara():
            ...
            operator.getitem(1, "hello")  # E: unsupported_operation
