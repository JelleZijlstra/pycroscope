...
class ClassAttributeChecker:
    ...
    def record_type_has_dynamic_attrs(self, typ: ClassKey) -> None:
        serialized = self.serialize_type(typ)
        if serialized is not None:
    ...
    def serialize_type(self, typ: ClassKey) -> object:
...
class NameCheckVisitor(node_visitor.ReplacingNodeVisitor):
    ...
    current_class_key: ClassKey | None
    ...
    final_class_keys: set[ClassKey]
    final_members_initialized_in_init: dict[ClassKey, set[str]]
    final_members_requiring_init: dict[ClassKey, dict[str, ast.AST]]
    enum_class_keys: set[ClassKey]
                ...
                if self._is_checking() and value is not None:
                    ...
                    if isinstance(existing, KnownValue) and isinstance(
                        ...
                        if (
                            ...
                            isinstance(value.typ, ClassOwner)
                            and stringify_object(type(existing.val)) == str(value.typ)
                        ):
                            current_scope.variables[varname] = value
                            return value, EMPTY_ORIGIN
    ...
    def _get_declared_symbol_initializer(
        self, type_object: TypeObject, name: str
    ) -> Value | None:
    ...
    def _prepare_synthetic_class_binding(
        ...
        *,
        ...
        if isinstance(
            self.current_class, (type, ClassOwner)
        ) and self._is_enum_class_key(self.current_class):
    ...
    def _populate_synthetic_class_from_scope(
        ...
        *,
        ...
        synthetic_type = synthetic_class.get_type_object(self.checker)
        ...
        dataclass_helpers.apply_synthetic_attributes(
            ...
            get_slot_names=self._dataclass_slot_names_from_type,
            get_field_parameters=self.checker.get_synthetic_dataclass_field_parameters,
        )
        self._apply_synthetic_enum_semantics(node, synthetic_type)
    ...
    def _make_updated_synthetic_symbol(
        ...
        *,
        ...
        dataclass_field = base.dataclass_field
        ...
        if resolved_property_info is not None:
            if initializer is _UNSET:
                resolved_initializer = (
                    base.initializer if base.property_info is not None else None
                )
        ...
        return ClassSymbol(
            ...
            initializer=(
                base.initializer
                if initializer is _UNSET and preserve_existing_initializer
    ...
    def _set_synthetic_member_on_type_object(
        self,
    ...
    def _set_synthetic_class_attribute(
        ...
        *,
        ...
        symbol = type_object.get_declared_symbol(synthetic_name)
        if (
            force_nonmember
    ...
    def _get_type_object_attribute_match_for_class_key(
        ...
        class_name = (
            ...
            else class_key.qualname.rsplit(".", maxsplit=1)[-1]
        )
    ...
    def _can_assign_to_base(
        self, base_value: Value, child_value: Value, base_class: ClassKey, node: ast.AST
    ) -> CanAssign:
    ...
    def _is_enum_class_key(self, class_key: ClassKey | None) -> bool:
    ...
    @contextlib.contextmanager
    def _set_current_class(self, current_class: ClassKey | None) -> Generator[None]:
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> Value:
        ...
        class_key: ClassKey = (
            class_obj if class_obj is not None else self._get_synthetic_class_key(node)
        )
        ...
        with ctx:
            ...
            if synthetic_typeddict is not None:
                ...
                if runtime_enum_fallback_class is not None:
                    ...
                    synthetic_owner = self._get_synthetic_class_key(node)
                    synthetic_class_type = TypedValue(synthetic_owner)
                    class_scope_object = synthetic_owner
            ...
            should_register_generic_bases = synthetic_typeddict is None and (
                isinstance(generic_class_key, ClassOwner)
                or bool(registered_type_param_values)
            )
            ...
            if (
                ...
                and isinstance(class_key, ClassOwner)
            ):
                namedtuple_fields = []
            ...
            if synthetic_typeddict is not None:
                ...
                if dataclass_semantics is not None:
                    dataclass_check_type = dataclass_metadata_class.get_type_object(
                        self.checker
                    )
            if (
                self._is_checking()
                and dataclass_semantics is not None
                and dataclass_check_type is not None
            ):
                self._check_dataclass_field_default_order(
                    node, dataclass_check_type, decorator_values
                )
                self._check_dataclass_post_init_signature(node, dataclass_check_type)
    ...
    def _apply_synthetic_enum_semantics(
        ...
        ignore_names = _enum_ignore_names(
            self._get_declared_symbol_initializer(synthetic_type, "_ignore_")
        )
        ...
        for member_name, statement in _iter_enum_assignment_candidates(node):
            ...
            value = self._get_declared_symbol_initializer(synthetic_type, member_name)
        ...
        runtime_enum = self._make_synthetic_enum_runtime_class(
            node,
            tuple(synthetic_type.get_direct_bases()),
    ...
    def _get_declared_enum_value_type(self, class_key: ClassKey | None) -> Value | None:
    ...
    def _get_dataclass_transform_info_from_class_key(
        self, class_key: ClassKey
    ) -> DataclassTransformInfo | None:
        info = self.make_type_object(class_key).get_direct_dataclass_transform_info()
        if info is not None or isinstance(class_key, ClassOwner):
            return info
    ...
    def _dataclass_slot_names_from_type(self, typ: ClassKey) -> tuple[str, ...] | None:
    ...
    def _slot_state_for_type(
        self, typ: ClassKey, *, seen: set[ClassKey] | None = None
    ...
    def _get_dataclass_status_for_type(self, typ: ClassKey) -> tuple[bool, bool | None]:
    ...
    def _check_dataclass_post_init_signature(
        self, node: ast.ClassDef, dataclass_type: TypeObject
    ) -> None:
        post_init_node = _get_dataclass_post_init_node(node)
        if post_init_node is None:
            return
        post_init_value = self._get_declared_symbol_initializer(
            dataclass_type, "__post_init__"
        )
    ...
    def _check_dataclass_field_default_order(
        self,
        node: ast.ClassDef,
        dataclass_type: TypeObject,
        decorator_values: DecoratorValues,
    ) -> None:
        dataclass_info = dataclass_type.get_direct_dataclass_info()
        ...
        field_records = dataclass_type.get_dataclass_fields()
    ...
    def _type_params_for_variance_class_key(
        self, class_key: ClassKey, arity: int
    ...
    def _type_parameter_class_key(self, value: Value) -> ClassKey | None:
        ...
        if isinstance(candidate, KnownValue):
            ...
            if not isinstance(candidate.val, type) and not isinstance(origin, type):
                return None
        elif not isinstance(
    ...
    def _is_protocol_class(
        self, base_values: Sequence[Value], class_scope_object: ClassKey | None
    ) -> bool:
        if isinstance(class_scope_object, (type, ClassOwner)):
    ...
    def _walk_values_for_type_param_collection(
        ...
        if (
            ...
            if self._is_class_object_attribute_root(value.root) is not True:
    ...
    def _check_inconsistent_generic_base_specialization(
        ...
        analyzed_bases = self._base_values_for_generic_analysis(node, base_values)
        seen_mappings: dict[ClassKey, TypeVarMap] = {}
        for base_node, base_value in zip(node.bases, analyzed_bases):
    ...
    def _is_valid_non_protocol_base_for_protocol(self, base_value: Value) -> bool:
        for candidate, _ in self._iter_type_parameter_base_candidates(base_value):
            if is_typing_name(candidate, "Generic"):
                return True
            if isinstance(candidate, ClassOwner):
    ...
    def _get_current_class_object(self, node: ast.ClassDef) -> type | None:
        ...
        if isinstance(self.current_class, type):
            nested_runtime_class = safe_getattr(self.current_class, node.name, None)
            if isinstance(nested_runtime_class, type):
                self._record_class_examined(nested_runtime_class)
                return nested_runtime_class
    ...
    def _class_key_is_type_subclass(
        self, class_key: ClassKey | None, seen: set[ClassKey] | None = None
    ) -> bool:
        if class_key is None:
            return False
        if seen is None:
    ...
    def _runtime_class_or_bases_use_self_annotations(self, typ: type) -> bool:
        ...
        result = self._type_object_or_bases_use_self_annotations(
            self.make_type_object(typ)
        )
    ...
    def _type_object_or_bases_use_self_annotations(
        self, tobj: TypeObject, seen: set[ClassKey] | None = None
    ) -> bool:
        if seen is None:
            seen = set()
        if tobj.typ in seen:
            ...
            if (
                ...
                uses_self_annotation = (
                    ...
                    or (
                        ...
                        and isinstance(enclosing_class.typ, ClassOwner)
                        and (
    ...
    def _validate_overload_block(
        ...
        in_protocol_class = (
            ...
            and isinstance(class_scope_object, (type, ClassOwner))
    ...
    def _finalize_synthetic_abstract_members(
        self, node: ast.ClassDef, class_key: ClassKey, *, is_protocol_class: bool
    ) -> None:
        if not self._is_checking() or not isinstance(class_key, ClassOwner):
            return
    ...
    def _required_abstract_members_for_base(self, class_key: ClassKey) -> set[str]:
        if isinstance(class_key, type):
    ...
    def _concrete_member_names_for_base(self, class_key: ClassKey) -> set[str]:
        if isinstance(class_key, type):
            provided = set(
    ...
    def _check_for_uninitialized_final_members(self, class_key: ClassKey) -> None:
        if not self._is_checking():
    ...
    def _is_local_synthetic_class_key(self, typ: ClassKey) -> bool:
        if not isinstance(typ, ClassOwner):
            return False
        if self.module is not None and self.module.__name__:
            return str(typ).startswith(f"{self.module.__name__}.")
        return str(typ).startswith(f"{self.filename}.")
    ...
    def _check_method_first_arg(
        ...
        if not isinstance(self.current_class, (type, ClassOwner)):
            return
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign) -> None:
        ...
        allowed_instance_final_annotation_target = False
        if is_final and isinstance(node.target, ast.Attribute):
            target_root_value = self.composite_from_node(node.target.value).value
            allowed_instance_final_annotation_target = (
                self._is_allowed_instance_final_annotation_target(
                    node.target, target_root_value
                )
            )
            if not allowed_instance_final_annotation_target:
                self._show_error_if_checking(
                    node.annotation,
    ...
    def _get_synthetic_instance_typevars(self, value: TypedValue) -> TypeVarMap:
        if not isinstance(value.typ, ClassOwner):
            return TypeVarMap()
    ...
    def _is_dynamic_function_attribute_target(self, root: SimpleType) -> bool:
        return (
            isinstance(root, CallableValue)
            or (
                isinstance(root, KnownValue)
                and isinstance(
                    root.val, (types.FunctionType, types.BuiltinFunctionType)
                )
            )
            or (
                isinstance(root, TypedValue)
                and root.typ in {types.FunctionType, types.BuiltinFunctionType}
            )
        )

    def _is_enum_value_assignment_on_current_receiver(
        ...
        return (
            ...
            is not None
            and class_keys_match(class_key, self.current_class_key)
        )
    ...
    def _check_attribute_write_on_simple_type(
        ...
        if attr is None:
            if (
                self.current_class_key is not None
                and self._is_current_method_receiver_node(node.value)
                and class_keys_match(tobj.typ, self.current_class_key)
            ...
            if tobj.is_protocol():
                self._show_error_if_checking(
                    node,
                    f"{root} has no attribute {node.attr!r}",
        ...
        if self.being_assigned is not None:
            ...
            if isinstance(can_assign, CanAssignError):
                self._show_error_if_checking(
                    ...
                    f"expected {expected_type}, got {self.being_assigned}",
                    error_code=ErrorCode.incompatible_assignment,
                    detail=str(can_assign),
                )
            else:
    ...
    def _is_protocol_base_member_assignment(
        self, class_key: ClassKey, member_name: str
    ) -> bool:
        for base_key in self.checker.get_generic_bases(class_key):
            if base_key == class_key:
                continue
    ...
    def _direct_base_class_keys(self, class_key: ClassKey) -> list[ClassKey]:
        if isinstance(class_key, ClassOwner):
            keys: list[ClassKey] = []
            for base_value in self.checker.make_type_object(
                class_key
    ...
    def _check_call_no_mvv(
        ...
        *,
        ...
        if (
            ...
            and isinstance(callee_wrapped.class_type.typ, ClassOwner)
    ...
    def _maybe_convert_local_namedtuple_class(
        ...
        synthetic_key = class_owner_from_key(synthetic_name)
        synthetic = self.checker.make_synthetic_class(synthetic_key)
        type_object = self.checker.make_type_object(synthetic_key)
    ...
    def signature_from_value(
        ...
        if (
            ...
            if synthetic_class is not None and type_object is not None:
                dataclass_helpers.apply_synthetic_attributes(
    ...
    def _record_type_has_dynamic_attrs(self, typ: ClassKey) -> None:
        if self.attribute_checker is not None:
    ...
    def _record_synthetic_attr_set(
        ...
        if synthetic_class is None:
            return
        self.checker.make_type_object(class_key).add_declared_symbol(
...
def _namedtuple_base_fields_from_base_values(
    base_values: Sequence[Value], checker: Checker
) -> set[str]:
    fields: set[str] = set()
    for base_value in base_values:
        for tobj in _iter_type_objects_from_base_value(base_value, checker):
