...
from .name_check_visitor import get_task_cls
...
def test_get_task_cls():
    assert get_task_cls(proxied_fn) is asynq.FutureBase
    assert get_task_cls(autogenerated) is asynq.AsyncTask
    assert get_task_cls(l0cached_async_fn) is asynq.AsyncTask
    assert get_task_cls(PropertyObject(1).l0cached_async_method) is asynq.AsyncTask
